...
class LoginForm(FlaskForm):
    ...
    submit = SubmitField('Submit login information')
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        return redirect(url_for('get_all_posts'))
    comments_list = Comment.query.filter_by(post_id=post_id)
    return render_template("post.html", post=requested_post, form=form,
                           current_user=current_user, comments_list=comments_list,
...
def comment():
    form = CommentForm()
    if form.validate_on_submit():
        commented = form.comment.data
        user_id = current_user.id
    return render_template('comment.html', logged_in=current_user.is_authenticated)
